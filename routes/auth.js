...
router.post('/register',async(req,res)=>{
    ...
    //Hash password 
     ...
     const hashPassword=await bcrypt.hash(req.body.password,salt);
            ...
            var sql="INSERT INTO user(user_id,password) VALUES('"+user_id+"','"+hashPassword+"')";
            
            await connection.query(sql, function (error, result,fields) {
...
router.post('/login',async(req,res)=>{
    ...
    // const hashPassword=await bcrypt.hash(req.body.password,salt);
            ...
            await connection.query(sql, function (error, result,fields) {
                ...
                if(result.length>0){
                    if(result[0].user_id===user_id)//&&result[0].password==password)
                    {
                        //connection.end();
